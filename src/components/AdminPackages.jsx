...
const AdminPackages = () => {
  ...
  const handleSeedDefaults = async () => {
    if (!window.confirm(i18n.language === 'ar' ? 'هل أنت متأكد من إضافة الباقات الافتراضية لقاعدة البيانات؟' : 'Are you sure you want to add default packages to the database?')) return;
    
    setLoading(true);
    const defaultPlans = [
      {
        type: 'nutrition',
        title: i18n.language === 'ar' ? 'خطة دايت' : 'Nutrition Plan',
      ...
      {
        type: 'training',
        title: i18n.language === 'ar' ? 'خطة تمرين' : 'Training Plan',
        duration: i18n.language === 'ar' ? '٣٠ يوم' : '30 DAYS',
      ...
      {
        type: 'vip',
        title: i18n.language === 'ar' ? 'خطة شاملة VIP' : 'VIP Plan',
    ...
    try {
      for (const plan of defaultPlans) {
        await addDoc(collection(db, "packages"), plan);
      }
      fetchPackages();
    } catch (error) {
      console.error("Error adding default packages", error);
      alert("Error adding default packages");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="admin-packages">
      ...
      <div className="admin-card">
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
          <h2 className="admin-card-title" style={{ marginBottom: 0 }}>{i18n.language === 'ar' ? 'الباقات المتاحة' : 'Active Packages'}</h2>
          <button 
            onClick={handleSeedDefaults} 
            style={{ padding: '0.6rem 1rem', background: '#f5a623', color: '#000', border: 'none', borderRadius: '4px', cursor: 'pointer', fontWeight: 'bold' }}
          >
            {i18n.language === 'ar' ? 'استيراد الباقات الافتراضية' : 'Import Default Packages'}
          </button>
        </div>
        {loading && packages.length === 0 ? (
          <p>{i18n.language === 'ar' ? 'جاري التحميل...' : 'Loading...'}</p>
        ) : packages.length === 0 ? (
          <p className="text-muted">{i18n.language === 'ar' ? 'لا يوجد باقات حالياً.' : 'No packages found.'}</p>
        ) : (
          <>
